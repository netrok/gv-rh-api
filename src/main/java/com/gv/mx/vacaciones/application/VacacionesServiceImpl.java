...
public class VacacionesServiceImpl implements VacacionesService {
    ...
    public VacacionesServiceImpl(PeriodoRepository p,
                                 SaldoRepository s,
                                 SolicitudRepository sr,
                                 FeriadoRepository fr,
                                 VacacionesMapper m){
        this.periodoRepo = p;
        this.saldoRepo = s;
        this.solRepo = sr;
    ...
    @Override
    public Page<SolicitudDTO> listarSolicitudes(Pageable p){
        return solRepo.findAll(p).map(mapper::toDto);
    }
    ...
    // Disponibilidad anual (para el controller)
    @Override
    public Disponibilidad disponibilidad(Long empleadoId, Integer anio){
        var s = saldoRepo.findByEmpleadoIdAndAnio(empleadoId, anio).orElseThrow();
        return new Disponibilidad(s.getDiasAsignados(), s.getDiasDisfrutados(), s.getDiasPendientes());
    }
    ...
    // L–V excluyendo feriados cuando incluirFeriadosExtra=false
